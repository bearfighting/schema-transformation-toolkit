...
describe("parser-json inference", () => {
  ...
  it("infers arrays of strings", () => {
    expect(
      inferJsonDocument('["a","b","c"]', "ArrayOfString")
    ).toEqual(schemaDocument("ArrayOfString", arrayType(scalarType("string"))));
  });
  ...
  it("promotes mixed numeric samples to number", () => {
    expect(
      inferJsonDocument('[1,2.5,3]', "MixedNumericArray")
    ).toEqual(schemaDocument("MixedNumericArray", arrayType(scalarType("number"))));
  });
  ...
  it("returns structured failures for mixed arrays without a common type", () => {
    ...
    expect(tryInferJsonDocument('[{"id":1},"a"]', "MixedObjectScalarArray")).toEqual({
      ok: false,
      code: "unsupported-mixed-types",
      message:
        "The input is valid JSON, but array elements do not share a common inferable type in AST v0."
  ...
  it("exposes prepared default parser options", () => {
    ...
    expect(preparedJsonSchemaParserOptions.resolved).toEqual(DEFAULT_JSON_PARSE_OPTIONS);
  });

  it("rejects future parse options that are not implemented yet", () => {
    expect(() =>
      inferJsonDocumentWithOptions('{"user_id":1}', {
        strictness: "best-effort"
      })
    ).toThrow(/strictness must currently be "strict"/);
  });
